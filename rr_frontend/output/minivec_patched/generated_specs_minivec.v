...
Section Vec_inv_t.
  ...
  Program Definition Vec_inv_t_inv_spec : ex_inv_def ((Vec_rt )) (list (place_rfn T_rt)) := mk_ex_inv_def
    (λ π inner_rfn 'xs, ∃ (cap : nat) (l : loc) (len : nat) (els : list _), ⌜inner_rfn = -[#((l, cap)); #(Z.of_nat $ len)]⌝ ∗ l ◁ₗ[π, Owned true] #(els) @ (◁ array_t (maybe_uninit T_ty) cap) ∗ ⌜xs = project_vec_els len els⌝ ∗ ⌜∀ i, (0 ≤ i < len)%nat → els !! i = Some (#(Some (xs !!! i)))⌝ ∗ ⌜∀ i, (len ≤ i < cap)%nat → els !! i = Some (#None)⌝ ∗ ⌜len = length xs⌝ ∗ ⌜len ≤ cap⌝ ∗ ⌜if decide (size_of_st (ty_syn_type T_ty) = 0%nat) then cap = Z.to_nat (max_int usize_t) else (size_of_array_in_bytes (ty_syn_type T_ty) cap ≤ max_int isize_t)%Z⌝ ∗ True)%I
    (λ π κ inner_rfn 'xs, ∃ (cap : nat) (l : loc) (len : nat) (els : _), ⌜inner_rfn = -[#((l, cap)); #(Z.of_nat $ len)]⌝ ∗ l ◁ₗ[π, Shared κ] #(els) @ (◁ array_t (maybe_uninit T_ty) cap) ∗ ⌜xs = project_vec_els len els⌝ ∗ ⌜∀ i, (0 ≤ i < len)%nat → els !! i = Some (#(Some (xs !!! i)))⌝ ∗ ⌜∀ i, (len ≤ i < cap)%nat → els !! i = Some (#None)⌝ ∗ ⌜len = length xs⌝ ∗ ⌜len ≤ cap⌝ ∗ ⌜if decide (size_of_st (ty_syn_type T_ty) = 0%nat) then cap = Z.to_nat (max_int usize_t) else (size_of_array_in_bytes (ty_syn_type T_ty) cap ≤ max_int isize_t)%Z⌝ ∗ True)%I
    ([] ++ (ty_lfts (T_ty)))
    ([] ++ (ty_wf_E (T_ty)))
    _ _ _
  .
...
Definition type_of_RawVec_T_grow (T_rt : Type) (T_st : syn_type) :=
  fn(∀ ((), ulft__) : 1 | (l, xs, cap, γ, T_ty) : (_ * _ * nat * _ * (type T_rt)), (λ ϝ, []); (#(l, cap), γ) @ (mut_ref (RawVec_inv_t (T_ty)) ulft__); (λ π : thread_id, (⌜ty_syn_type T_ty = T_st⌝) ∗ (⌜(size_of_array_in_bytes (ty_syn_type T_ty) (2 * cap) ≤ max_int isize_t)%Z⌝) ∗ (⌜(size_of_st (ty_syn_type T_ty) > 0)%Z⌝) ∗ (l ◁ₗ[π, Owned false] #xs @ (◁ array_t (maybe_uninit T_ty) cap)) ∗ (⌜ty_allows_writes T_ty⌝) ∗ (⌜ty_allows_reads T_ty⌝) ∗ ty_sidecond T_ty))
    → ∃ (new_cap, l') : (nat * loc), () @ unit_t; (λ π : thread_id, (gvar_pobs γ (l', new_cap)) ∗ (l' ◁ₗ[π, Owned false] #(xs ++ replicate (new_cap - cap) #None) @ (◁ array_t (maybe_uninit T_ty) new_cap)) ∗ (⌜new_cap > cap⌝) ∗ ⌜(size_of_array_in_bytes T_st new_cap ≤ max_int isize_t)%Z⌝).

Definition type_of_Vec_T_insert (T_rt : Type) (T_st : syn_type) :=
  fn(∀ ((), ulft__) : 1 | (xs, γ, i, x, T_ty) : (_ * _ * nat * _ * (type T_rt)), (λ ϝ, []); (#xs, γ) @ (mut_ref (Vec_inv_t (T_ty)) ulft__), Z.of_nat i @ (int USize), x @ T_ty; (λ π : thread_id, (⌜ty_syn_type T_ty = T_st⌝) ∗ (⌜i ≤ length xs⌝) ∗ (⌜(length xs < max_int usize_t)%Z⌝) ∗ (⌜(size_of_array_in_bytes (ty_syn_type T_ty) (2 * length xs) ≤ max_int isize_t)%Z⌝) ∗ (⌜ty_allows_writes T_ty⌝) ∗ (⌜ty_allows_reads T_ty⌝) ∗ ty_sidecond T_ty))
...
Definition type_of_Vec_T_push (T_rt : Type) (T_st : syn_type) :=
  fn(∀ ((), ulft__) : 1 | (xs, γ, x, T_ty) : (_ * _ * _ * (type T_rt)), (λ ϝ, []); (#xs, γ) @ (mut_ref (Vec_inv_t (T_ty)) ulft__), x @ T_ty; (λ π : thread_id, (⌜ty_syn_type T_ty = T_st⌝) ∗ (⌜(length xs < max_int usize_t)%Z⌝) ∗ (⌜(size_of_array_in_bytes (ty_syn_type T_ty) (2 * length xs) ≤ max_int isize_t)%Z⌝) ∗ (⌜ty_allows_writes T_ty⌝) ∗ (⌜ty_allows_reads T_ty⌝) ∗ ty_sidecond T_ty))
...
Definition type_of_RawVec_T_new (T_rt : Type) (T_st : syn_type) :=
  fn(∀ (()) : 0 | (T_ty) : ((type T_rt)), (λ ϝ, []); (λ π : thread_id, (⌜ty_syn_type T_ty = T_st⌝) ∗ (⌜ty_allows_writes T_ty⌝) ∗ (⌜ty_allows_reads T_ty⌝) ∗ ty_sidecond T_ty))
    → ∃ (l, cap) : (loc * nat), (l, cap) @ (RawVec_inv_t (T_ty)); (λ π : thread_id, (⌜cap = if decide (size_of_st (ty_syn_type T_ty) = 0%nat) then Z.to_nat (max_int usize_t) else 0%nat⌝) ∗ (l ◁ₗ[π, Owned false] #(replicate cap #None) @ (◁ array_t (maybe_uninit T_ty) cap))).
