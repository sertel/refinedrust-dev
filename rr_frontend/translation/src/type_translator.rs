...
impl<'tcx, 'def> From<ParamScope<'tcx, 'def>> for radium::GenericScope<'def> {
...
impl<'tcx, 'def> ParamScope<'tcx, 'def> {
    ...
    pub fn new_from_generics(x: ty::GenericArgsRef<'tcx>, with_origin: Option<(ty::TyCtxt<'tcx>, DefId)>) -> Self {
        ...
        -> Result<(), TranslationError<'tcx>>
    {
        ...
        for trait_ref in &requirements {
            ...
            if let Some(trait_spec) = trait_registry.lookup_trait(trait_ref.def_id) {
                ...
                //  If we haven't ordered them in the right way, we might be unable to translate
                //  them correctly

                let mut deps = HashSet::new();
                // the scope to translate the arguments in
                let mut state = TranslationStateInner::TranslateAdt(AdtTranslationState::new(&mut deps, &*self, &param_env));
        ...
        for trait_ref in requirements {
            ...
            let key = (trait_ref.def_id, generate_args_inst_key(env.tcx(), trait_ref.args).unwrap());
            if let Some(trait_use) = self.trait_scope.used_traits.get_mut(&key) {
...
impl<'a, 'tcx, 'def> AdtTranslationState<'a, 'tcx, 'def> {
    pub fn new(deps: &'a mut HashSet<DefId>, scope: &'a ParamScope<'tcx, 'def>, param_env: &'a ty::ParamEnv<'tcx>) -> Self {
        Self {
            deps,
            scope,
            param_env
...
impl<'a, 'def, 'tcx> TranslationStateInner<'a, 'def, 'tcx> {
    ...
    fn normalize_type_erasing_regions(&self, tcx: ty::TyCtxt<'tcx>, ty: Ty<'tcx>) -> Result<Ty<'tcx>, TranslationError<'tcx>> {
        match self {
            Self::InFunction(ref state) => {
                normalize_erasing_regions_in_function::<Ty<'tcx>>(state.did, tcx, ty)
            },
            Self::TranslateAdt(state) => {
                tcx.try_normalize_erasing_regions(*state.param_env, ty)
    ...
    fn lookup_trait_use(&self, tcx: ty::TyCtxt<'tcx>, trait_did: DefId, args: &[ty::GenericArg<'tcx>]) -> Result<&GenericTraitUse<'def>, TranslationError<'tcx>> {
        match self {
            Self::InFunction(ref state) => {
    ...
    fn lookup_ty_param(&self, param_ty: &ty::ParamTy) -> Result<radium::Type<'def>, TranslationError<'tcx>> {
        match self {
            ...
            Self::CalleeTranslation(state) => {
                let ty =
                    state.param_scope.lookup_ty_param_idx(param_ty.index as usize).ok_or_else(|| {
                        TranslationError::UnknownVar(format!(
                            "unknown generic parameter {:?}",
                            param_ty
                        ))
    ...
    fn lookup_early_region(&self, region: &ty::EarlyBoundRegion) -> Result<radium::Lft, TranslationError<'tcx>> {
        match self {
            TranslationStateInner::InFunction(scope) => {
                info!("Looking up lifetime {region:?} in scope {:?}", scope.lifetime_scope);
                let lft = scope.lifetime_scope.lookup_early_region(region.index as usize).ok_or_else(||
                    TranslationError::UnknownEarlyRegion(region.clone())
                )?;
                Ok(lft.to_owned())
            },
            TranslationStateInner::TranslateAdt(scope) => {
                ...
                return Err(TranslationError::UnknownEarlyRegion(region.clone()));
            },
            TranslationStateInner::CalleeTranslation(_) => {
                Ok("DUMMY".to_owned())
            },
        }
    }
    ...
    fn lookup_late_region(&self, region: usize) -> Result<radium::Lft, TranslationError<'tcx>> {
        match self {
            TranslationStateInner::InFunction(scope) => {
                info!("Looking up lifetime {region:?} in scope {:?}", scope.lifetime_scope);
                let lft = scope.lifetime_scope.lookup_late_region(region).ok_or_else(|| 
                    TranslationError::UnknownLateRegion(region)
                )?;
                Ok(lft.to_owned())
            },
            TranslationStateInner::TranslateAdt(scope) => {
        ...
        match self {
            TranslationStateInner::InFunction(scope) => {
                if let Some(info) = scope.polonius_info {
                    ...
                    let r = scope.lifetime_scope.lookup_region(v).ok_or_else(||
                        TranslationError::UnknownPoloniusRegion(v)
                    )?;
                    info!("Translating region: ReVar {:?} as {:?}", v, r);
                    Ok(r.to_owned())
                }
...
impl<'def, 'tcx: 'def> TypeTranslator<'def, 'tcx> {
    ...
    pub fn translate_region_in_scope(scope: &ParamScope<'tcx, 'def>, region: ty::Region<'tcx>) -> Result<radium::Lft, TranslationError<'tcx>> {
        let mut deps = HashSet::new();
    ...
    fn register_struct(
        ...
        let translate_adt = || {
            ...
            let (variant_def, invariant_def) =
                self.make_adt_variant(&struct_name, ty, adt, AdtTranslationState::new(&mut deps, &scope, &param_env))?;
    ...
    fn make_adt_variant<'a>(
        ...
        for f in &ty.fields {
            ...
            let f_ty = self.env.tcx().type_of(f.did).instantiate_identity();
            let mut ty =
                self.translate_type_in_state(f_ty, &mut TranslationStateInner::TranslateAdt(AdtTranslationState::new(&mut adt_deps.deps, &adt_deps.scope, &adt_deps.param_env)))?;
    ...
    fn register_enum(&self, def: ty::AdtDef<'tcx>) -> Result<(), TranslationError<'tcx>> {
        ...
        let translate_variants = || {
            ...
            for v in def.variants() {
                ...
                let (variant_def, invariant_def) =
                    self.make_adt_variant(struct_name.as_str(), v, def, AdtTranslationState::new(&mut deps, &scope, &param_env))?;
    ...
    pub fn translate_type_in_state<'a, 'b>(
        ...
        match ty.kind() {
            ...
            TyKind::Param(param_ty) => {
                state.lookup_ty_param(param_ty)
            },
            ...
            TyKind::Alias(kind, alias_ty) => {
                ...
                match kind {
                    ty::AliasKind::Projection => {
                        ...
                        let trait_use = &state.lookup_trait_use(self.env.tcx(), trait_did, alias_ty.args)?.trait_use;
...
impl<'def, 'tcx> LocalTypeTranslator<'def, 'tcx> {
    ...
    pub fn lookup_trait_param(
        ...
        let scope = self.scope.borrow();
        let trait_use = scope.generic_scope.trait_scope.lookup_trait_use(env.tcx(), trait_did, args)?;
        Ok(trait_use.clone())
    }
    ...
    pub fn register_use_trait_procedure(
        ...
        let trait_spec_use = {
            let scope = self.scope.borrow();
            scope.generic_scope.trait_scope.lookup_trait_use(env.tcx(), trait_did, trait_args.as_slice())?.trait_use.clone()
        };
