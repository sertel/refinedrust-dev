...
use rustc_middle::mir::{StatementKind, Rvalue, Place, Local, BasicBlock, BasicBlockData, Body, TerminatorKind};
...
impl<'def, 'tcx> CheckedOpLocalAnalysis<'def, 'tcx> { 
    ...
    fn is_checked_op(&self, val: &Rvalue<'tcx>) -> bool {
        if let Rvalue::CheckedBinaryOp(_, _) = *val {
            true
        }
        else {
            false
    ...
    fn successors_of_bb(&self, bb: &BasicBlockData<'tcx>) -> Vec<BasicBlock> {
        ...
        if let Some(ref term) = bb.terminator {
            match &term.kind {
                TerminatorKind::Call { func: _, args: _, destination: _, target, .. } => {
